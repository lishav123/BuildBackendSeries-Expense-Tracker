...
class ExpenseTracker:
    ...
    def add_expense(self, description: str, expense: float, category: str = "General"):
        if self.data == []:
            self.data.append({"id": 0, "description": description, "expense": expense, "date": str(datetime.date(datetime.now()).isoformat()), "category": category.lower()})

        else:
            self.data.append({"id": self.data[-1]["id"] + 1, "description": description, "expense": expense, "date": str(datetime.date(datetime.now()).isoformat()), "category": category.lower()})
    ...
    def summary_expense(self, month: int | None = None, category: str | None = None):
        ...
        if month is None:
            total_expense = sum(list(map(lambda x: x["expense"], self.data))) if category is None else sum(list(map(lambda x: x["expense"], list(filter(lambda x: x["category"] == category, self.data)))))
...
if __name__ == '__main__':
    ...
    tracker.add_expense(description="Test #2", expense=200, category="food")
    tracker.add_expense(description="Test #3", expense=300, category="food")
    tracker.add_expense(description="Test #4", expense=400)
    tracker.add_expense(description="Test #5", expense=500, category="food")
    tracker.add_expense(description="Test #6", expense=600)
    tracker.add_expense(description="Test #7", expense=700)
    tracker.add_expense(description="Test #8", expense=800, category="payment")
    tracker.add_expense(description="Test #9", expense=900, category="payment")
